...
if __name__ == '__main__':
    ...
    # 实例化一个对象 module = Module()
    cnn_score = Fai_score(args, FILTER_NUM, FILTER_WINDOW, WINDOW_DOC, WINDOW_CONTEXT, WINDOW_TITLE, EMBEDDING, LAMDA)
    cnn_score = cnn_score.cuda()
    pretrain = Local_Fai_score(args, FILTER_NUM, FILTER_WINDOW, WINDOW_DOC, WINDOW_CONTEXT, WINDOW_TITLE, EMBEDDING, LAMDA)
    torch.save(pretrain.state_dict(),LOCAL_MODEL_LOC)
    ...
    for epoch in range(EPOCH):
        ...
        file_count = 0
        loss_sum = 0
        for train_batched in tqdm(data_loader_train):
        #for i_batch, train_batched in enumerate(data_loader_train):
            cnn_score.train()
            file_count += 1
            ...
            #     k, FINETUNE)
            filename = train_batched['filename'][0]
            if filename.find(" ") >= 0:
         ...
         #       print(filename)
            ...
            y_true = torch.Tensor(train_batched['y_label'][0].numpy())
            s, s_global, local_score, fai_local_score_softmax, global_score = cnn_score(args, train_batched, RANDOM_K,
                                                                                        LAMDA,
            ...
            y_true_index = []
            for y_t_i in range( train_batched['m']):
                for y_t_j in range(train_batched['n']):
            ...
            if file_count % TEAM == 0 or file_count == TrainFileNum:
                ...
                for idx_batch, val_batch in enumerate(data_loader_val):
                    ...
                    if val_batch['m'] != int(doc_men[filename]):
                        ...
                        print("erooooooor!!")
                    s, s_global, fai_score, fai_local_score_softmax, global_score = cnn_score(args,val_batch, RANDOM_K,
                                                                                              LAMDA, MODEL_FLAG,
                                                                                              pos_embed_dict,
                                                                                              entity_embed_dict)
                    ...
                    for i in range(val_batch['m']):
                        if int(val_batch['y_label'][0][i][int(y_forecast_local[i])]) == 1:
                            count_true += 1
                            correct_temp += 1
                    y_true = []
                ...
                print("#######Evaluate test#######")
                for i in range(2, 8):
                    acc, eval_mi_prec, eval_ma_prec, eval_mi_rec, eval_ma_rec, eval_mi_f1, eval_ma_f1 = eval(args, 
                                                                                                             ROOT,
                                                                                                             cnn_score,
                                                                                                             doc_men,
                                                                                                             doc_totalmen,

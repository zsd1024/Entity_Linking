...
if __name__ == '__main__':
  ...
  #  print(data_loader_train)
    ...
    cnn_score = Local_Fai_score(args, FILTER_NUM, FILTER_WINDOW, WINDOW_DOC, WINDOW_CONTEXT, WINDOW_TITLE, EMBEDDING, LAMDA)
    ...
    for epoch in range(EPOCH):
      ...
      # print(len(iterator.next()))
      #  new_mentions,gold_entities_ind,docment,new_mentions_surface,new_mentions_context,new_docment_vec,new_entities_Tvec,new_entities_Bvec,new_mentions2entities,new_entities2entities,new_entity_sr,sparse_features = iterator.next()
        for train_batch in tqdm(data_loader_train):      
        #for i_batch,train_batch in enumerate(data_loader_train):
            #print("i",i_batch, train_batch['mention_vec'].size(), sample_batched['y_label'].size())            
            #print('type',type(train_batch))
            
            #print(list(train_batch)[0])
            #print(train_batch)
            file_count += 1
            #print('type2',train_batch['mention_vec'][0].size())
            #print('type3',len(train_batch['mention_vec']))
            #print('type5',train_batch['m'])
            cnn_score.train()
            y_true = torch.Tensor(train_batch['y_label'][0].numpy())
            local_score, local_score_softmax, local_score_uniform = cnn_score(args,train_batch)
            y_true_index = []
            for y_t_i in range(train_batch['m']):
                for y_t_j in range(train_batch['n']):
                    if int(y_true[y_t_i][y_t_j]) == 1:
                        y_true_index.append(y_t_j)
            #print('len',train_batch['m'])
            #print('len',train_batch['n'])
            y_true_index = Variable(torch.LongTensor(y_true_index)).cuda()
            #print('ytrue',y_true_index)
            #print('local',local_score)
            loss = loss_function(local_score, y_true_index)
            loss_sum += loss.cuda().data
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()

            # testing
            if file_count % TEAM == 0 or file_count == TrainFileNum:
                cnn_score.eval()
                count_true = 0
                count_label = 0
                total_mentions = []
                ...
                test_file_c = 0
                for idx_batch, val_batch in enumerate(data_loader_val):
                    cur_stream = []
                    correct_temp = 0
                    ...
                    #     val_batched,len(val_batched),FINETUNE)
                    # batch = extract_data_from_dataloader(val_batched, len(val_batched), FINETUNE)
                    if "testa " not in val_batch['filename'][0]: continue
                    test_file_c += 1
                    y_true = torch.Tensor(val_batch['y_label'][0].numpy())
                    filename = val_batch['filename'][0]
                    if filename.find(" ") >= 0:
                        if filename.split(" ")[1] == "S":
                            filename = val_batch['filename'][0].replace("S", "S.")
                        if filename.split(" ")[1] == "N":
                            filename = filename.replace("N", "N.")

                    if filename.find("U.N") >= 0:
                        filename = filename.replace("U.N", "U.N.")
                        print(val_batch['filename'][0])
                    ...
                    if filename.find("O.J") >= 0:
                        ...
                        print(filename)
                    if val_batch['m'] != int(doc_men[filename]):
                        print(str(val_batch['m']) + "|||" + str(doc_men[filename]))
                        print("erooooooor!!")
                    fai_local_score_temp, fai_local_score_softmax, fai_local_score_uniform = cnn_score(args, val_batch)
                    fai_score = fai_local_score_temp.cpu().data
                    y_forecast = []
                    y_local = []
                    count_label += val_batch['m']
                    for i in range(val_batch['m']):
                        y_forecast.append(np.argmax(fai_score[i].numpy()))
                    for i in range(val_batch['m']):
                        if int(val_batch['y_label'][0][i][int(y_forecast[i])]) == 1:
                            count_true += 1
                            correct_temp += 1
                    y_true = []
                    ...
                    for i in range(val_batch['m']):
                        y_true_temp = []
                        for j in range(val_batch['n']):
                            if (int(val_batch['y_label'][0][i][j]) == 1):
                                y_true_temp.append(j)
                        y_true.append(y_true_temp)
                acc, eval_mi_prec, eval_ma_prec, eval_mi_rec, eval_ma_rec, eval_mi_f1, eval_ma_f1 = Fmeasure(
                    count_true,
                    count_label,
                    actual_mentions,
                    total_mentions,
                ...
                print("time:" + str((endtime - starttime).total_seconds()) + "|||epoch:" + str(
                    epoch_count) + "|||step:" + str(file_count) + "|||loss:" + str(
                    float(loss_sum)) + "|||acc:" + str(
                    acc))
                print("eval_mi_prec:" + str(eval_mi_prec) + "|||eval_mi_rec:" + str(
                        eval_mi_rec) + "|||eval_mi_f1:" + str(
                        eval_mi_f1))
                print("eval_ma_prec:" + str(eval_ma_prec) + "|||eval_ma_rec:" + str(
                ...
                print("time:" + str((endtime - starttime).total_seconds()))
                for i in range(2, 8):
                    eval(args, ROOT, cnn_score, doc_men, doc_totalmen, data_loader_test, i, True, None, 0, 0, 0)
print('over')

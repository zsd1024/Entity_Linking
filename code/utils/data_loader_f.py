...
class DataSet(data.Dataset):
    def __init__(self,root,windows,doc_num,body_num,val,test):
        ...
        with open(os.path.join(self.root, docname),'rb') as f:
            ...
            for i in docname_list:
                #print(i)
                if i.endswith("."):
                    p = docname_list.index(i)
                    #print(p)
                    docname_list[p]= i[0:-1]
    ...
    def __getitem__(self, index):
        ...
        #print(docment)
        vocab=self.vocab
        if docment.find("U.S")>=0:
           docment = docment.replace("U.S","U.S.")
          # print(docment)
        if docment.find("U.N")>=0:
           docment = docment.replace("U.N","U.N.")
           #print(docment)
        if docment.find("O.J")>=0:
           docment = docment.replace("O.J","O.J.")
           #print(docment)
        if docment.find(" ")>=0:
           if docment.split(" ")[1]== "S":
               docment = docment.replace("S","S.")
           if docment.split(" ")[1]== "N":
               docment = docment.replace("N","N.")
        ...
        #with open(os.path.join(self.root,'pkl','entity_embed',docment+'.pkl'),'rb') as fr_sp:
            sp=pickle.load(fr_sp)            
            #print(os.path.join(self.root,'features',docment))
       ...
       # print(type(new_mentions_surface))
...
def collate_fn(data):
    new_mentions,gold_entities_ind,docment,new_mentions_surface,new_mentions_context,new_docment_vec,new_entities_Tvec,new_entities_Bvec,new_mentions2entities,new_entities2entities,new_entity_sr,sparse_features = zip(*data)
  
    dump = dict([('y_label', gold_entities_ind),
                 ('mention_entity', new_mentions2entities),
                 ('entity_entity', new_entities2entities),
                 ('SR', new_entity_sr),
                 ('n',len(list(new_entities2entities[0]))),
                 ('m',len(list(new_mentions[0]))),
                 ('mention', new_mentions),
                 ('mention_vec', new_mentions_surface),
                 ('context_vec', new_mentions_context),
                 ('doc_vec', new_docment_vec),
                 ('title_vec', new_entities_Tvec),
                 ('body_vec', new_entities_Bvec),
                 ('filename', docment),
                 ('sfeats', sparse_features)])
    #print('len',len(data))
...
def get_loader(root,windows,doc_num,body_num,val,test, shuffle, num_workers):
   ...
   #     sample = data[i]
   #     print('i',i,sample['y_label'].shape,sample['mention_vec'].shape)
